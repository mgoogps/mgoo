...
(function (e) {
    if (typeof define !== "undefined" && define.amd) {
        ...
        var t = require("jquery"); module.exports = e(t)
    } else {
...
(function (e) {
    ...
    //} function y() {
    //    t.viewportTop = n.scrollTop(); t.viewportBottom = t.viewportTop + t.viewportHeight; t.documentHeight = r.height(); if (t.documentHeight !== d) { g = i.length; while (g--) { i[g].recalculateLocation() } d = t.documentHeight }
    //} function b() {
    //    t.viewportHeight = m(); y(); x()
    //} function E() {
    //    clearTimeout(w); w = setTimeout(b, 100)
    //} function x() {
    //    S = i.length; while (S--) {
    //        i[S].update()
    //    } S = i.length; while (S--) { i[S].triggerCallbacks() }
    //} function T(n, r) {
    //    function x(e) {
    //        if (e.length === 0) {
    //            return
    //        } E = e.length; while (E--) {
    //            S = e[E]; S.callback.call(i, v); if (S.isOne) {
    //                e.splice(E, 1)
    //            }
    //        }
    //    } var i = this; this.watchItem = n; if (!r) { this.offsets = p } else if (r === +r) {
    //        this.offsets = {
    //            top: r, bottom: r
    //        }
    //        } else { this.offsets = e.extend({}, p, r) } this.callbacks = {}; for (var d = 0, m = h.length; d < m; d++) {
    //            i.callbacks[h[d]] = []
    //        } this.locked = false; var g; var y; var b; var w; var E; var S; this.triggerCallbacks = function () {
    //            if (this.isInViewport && !g) {
    //                x(this.callbacks[o])
    //            } if (this.isFullyInViewport && !y) { x(this.callbacks[u]) } if (this.isAboveViewport !== b && this.isBelowViewport !== w) {
    //                x(this.callbacks[s]);
    //                if (!y && !this.isFullyInViewport) {
    //                    x(this.callbacks[u]); x(this.callbacks[f])
    //                } if (!g && !this.isInViewport) {
    //                    x(this.callbacks[o]); x(this.callbacks[a])
    //                }
    //            } if (!this.isFullyInViewport && y) {
    //                x(this.callbacks[f])
    //            } if (!this.isInViewport && g) { x(this.callbacks[a]) } if (this.isInViewport !== g) {
    //                x(this.callbacks[s])
    //            }
    //            switch (true) {
    ...
    //            } g = this.isInViewport; y = this.isFullyInViewport; b = this.isAboveViewport; w = this.isBelowViewport
    //        }; this.recalculateLocation = function () {
    //            if (this.locked) { return } var n = this.top; var r = this.bottom; if (this.watchItem.nodeName) {
    //                var i = this.watchItem.style.display; if (i === "none") { this.watchItem.style.display = "" } var s = e(this.watchItem).offset();
    //                this.top = s.top; this.bottom = s.top + this.watchItem.offsetHeight;
    //                if (i === "none") {
    //                    this.watchItem.style.display = i
    //                }
    //            } else if (this.watchItem === +this.watchItem) {
    //                if (this.watchItem > 0) { this.top = this.bottom = this.watchItem } else {
    //                    this.top = this.bottom = t.documentHeight - this.watchItem
    //                }
    //            } else {
    //                this.top = this.watchItem.top; this.bottom = this.watchItem.bottom
    //            } this.top -= this.offsets.top; this.bottom += this.offsets.bottom; this.height = this.bottom - this.top; if ((n !== undefined || r !== undefined) && (this.top !== n || this.bottom !== r)) { x(this.callbacks[l]) }
    //        }; this.recalculateLocation(); this.update(); g = this.isInViewport; y = this.isFullyInViewport; b = this.isAboveViewport; w = this.isBelowViewport
    //} function O(e) {
    //    v = e; y(); x()
    //} var t = {};
    //var n = e(window); var r = e(document);
    //var i = []; var s = "visibilityChange"; var o = "enterViewport"; var u = "fullyEnterViewport";
    //var a = "exitViewport"; var f = "partiallyExitViewport"; var l = "locationChange"; var c = "stateChange";
    //var h = [s, o, u, a, f, l, c];
    //var p = { top: 0, bottom: 0 };
    //t.viewportTop; t.viewportBottom;
    //t.documentHeight; t.viewportHeight = m();
    //var d; var v; var g; var w; var S;
    //T.prototype = {
    //    on: function (e, t, n) {
    //        switch (true) {
    ...
    //        } else { throw new Error("Tried to add a scroll monitor listener of type " + e + ". Your options are: " + h.join(", ")) }
    //    }, off: function (e, t) {
    //        if (this.callbacks[e]) {
    //            for (var n = 0, r; r = this.callbacks[e][n]; n++) {
    //                if (r.callback === t) {
    //                    this.callbacks[e].splice(n, 1); break
    //                }
    //            }
    //        } else {
    ...
    //    }, update: function () {
    //        this.isAboveViewport = this.top < t.viewportTop; this.isBelowViewport = this.bottom > t.viewportBottom; this.isInViewport = this.top <= t.viewportBottom && this.bottom >= t.viewportTop; this.isFullyInViewport = this.top >= t.viewportTop && this.bottom <= t.viewportBottom || this.isAboveViewport && this.isBelowViewport
    //    }, destroy: function () { var e = i.indexOf(this), t = this; i.splice(e, 1); for (var n = 0, r = h.length; n < r; n++) { t.callbacks[h[n]].length = 0 } }, lock: function () { this.locked = true }, unlock: function () { this.locked = false }
    //}; var N = function (e) {
    //    return function (t, n) {
    //        this.on.call(this, e, t, n)
    //    }
    //}; for (var C = 0, k = h.length; C < k; C++) {
    //    var L = h[C]; T.prototype[L] = N(L)
    //} try {
    //    y()
    //} catch (A) {
    //    e(y)
    //} n.on("scroll", O); n.on("resize", E);
    //t.beget = t.create = function (t, n) {
    //    if (typeof t === "string") {
    //        t = e(t)[0]
    //    } if (t instanceof e) {
    //        t = t[0]
    //    } var r = new T(t, n); i.push(r); r.update(); return r
    //}; t.update = function () {
    //    v = null; y(); x()
    //}; t.recalculateLocations = function () {
    //    t.documentHeight = 0; t.update()
    //}; return t
})
